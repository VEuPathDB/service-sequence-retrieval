...
# Test asynchronous gene tree generation with newick format output
...
echo "Submitting async gene tree job..."
...
# Check for unexpected guide tree (gene tree should not produce guide tree file)
if echo "$files" | jq -e '.[] | select(. == "guidetree.dnd")' > /dev/null 2>&1; then
  echo -e "\nERROR: Guide tree file found for GENETREE format!"
  exit 1
fi

echo -e "\n=== Gene Tree Output (Newick format) ==="
