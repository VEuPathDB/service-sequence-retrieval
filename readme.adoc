...
As FgpUtil is not a standard versioned dependency it is built separately.  By
default, the version of FgpUtil used is the latest commit to main/master.

In development this may not be an issue, but it can cause problems for later
automated rebuilds if breaking changes are made to FgpUtil.

To lock FgpUtil to a specific version, the following block must be added to your
project's `build.gradle.kts` file.

[source, kotlin]
----
containerBuild {
  setFgpUtilVersion("<Some branch, tag, or commit hash>")
}
----
